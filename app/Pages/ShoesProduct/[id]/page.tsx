...
type productType = typeof SneakerData[0] & {
  secondImage?: string;
  lessPrice?: string;
  off?: string;
};
...
const ProductDetails = () => {
  ...
  const { id } = params;
  const product = (SneakerData as productType[]).find((product) => product.id.toString() === id);
    ...
    <>
      ...
      <div className='px-[5%] lg:px-[10%] py-20 pt-20 flex flex-col lg:flex-row gap-10 border-b border-gray-300'>
        ...
        <div className='w-full lg:w-1/2 flex flex-col gap-5'>
          <div>
            <span className='text-gray-700 bg-prim px-3 py-1 rounded-full font-semibold uppercase'>
              {product.brand}
            </span>
          </div>
          ...
          <div className='flex items-center gap-4'>
            ...
            {product.lessPrice && <span className='line-through text-gray-500'>{product.lessPrice}</span>}
            {product.off && <span className='bg-red-500 text-white px-3 rounded Exo font-medium'>{product.off}</span>}
          </div>

          <p className='text-gray-600 mt-5'>
            Discover the perfect blend of style and comfort with our latest footwear collection. Crafted with premium materials and designed for durability, these shoes are perfect for any occasion.
          </p>

          <p className='text-gray-600'>
            Experience superior cushioning and support with every step. Whether you're hitting the gym or the streets, our shoes provide the performance you need with the look you want.
          </p>

          <div className='flex items-center gap-2'>
            <h3 className='Exo tracking-wide font-semibold uppercase'>Size &nbsp; -</h3>
          ...
          <p>
            Categories: Shoes, Sneakers, Running, Training, Lifestyle, Casual.
          </p>
        </div>
      </div>
      ...
      <div className='px-[5%] lg:px-[10%] py-20'>
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
          <div>
            ...
            <p className='text-gray-600 tracking-wide'>
              Elevate your game with our high-performance footwear. Engineered for maximum comfort and speed, these shoes feature a breathable mesh upper and a responsive midsole that provides energy return with every stride. The durable rubber outsole offers exceptional traction on various surfaces, making them ideal for both indoor and outdoor activities.
            </p>

            <p className='text-gray-600 tracking-wide mt-4'>
              Whether you're a professional athlete or a weekend warrior, our shoes are designed to help you reach your full potential. With a sleek and modern design, they're not just about performance – they're also about style. Join the movement and experience the difference of quality craftsmanship.
            </p>
          </div>
        ...
        >
          {SneakerData.map((product: productType, index) => {
            ...
            return (
              <SwiperSlide key={index} className='sneaker-card product-card group'>
                <div className="relative aspect-3/4 overflow-hidden rounded-3xl bg-gray-100 mb-6 group cursor-pointer">
                  <Link href={`/Pages/ShoesProduct/${product.id}`} className="block h-full w-full">
                    <Image
                      src={product.image}
                      alt={product.title}
                      fill
                ...
                <div className="flex flex-col grow px-2">
                  ...
                  <Link href={`/Pages/ShoesProduct/${product.id}`} className="group-hover:text-gray-600 transition-colors">
